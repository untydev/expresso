...
import { rewrite, validate } from '../utils.js'
...
function makeRespond (req, res, next) {
  return (how) => {
    const { AppError, logger } = req.service
    if (how.view) {
      ...
      /* TODO: Set content type based on the file extension. Decide what to do
       * when the extension is missing, unknown or doesn't have the corresponding
       * mime type. Provide an option for clients to manually specify mime type. */

      /* TODO: Add options to control caching behavior through HTTP headers,
       * like maxAge, immutable, lastModified, etag, etc. */
      res.type('text/html')
      const path = nodePath.resolve(how.file)
      logger.detail(`Send: ${how.file} => ${path}`)
      res.sendFile(path)
      ...
      throw new AppError(`Unrecognized response type in a call to 'req.respond'`, AppError.API_INVALID_PARAMS)
    }
  }
}
...
export async function useRoutes (context) {
  ...
  router.use((req, res, next) => {
    ...
    req.service.respond = makeRespond(req, res, next)
    next()
  })
